...
class CommandGenerator {
    ...
    Command generate(Map params, Closure closure) {
        ...
        List<byte[]> usedClosuresBytes = params.usedClosures.collect { getClassBytes(it.class) }
        List<byte[]> supportsUsedClosures = params.usedClosures.collect { getSupportingClassesBytes(it.class) }.inject([]) { flattened, current -> flattened + current }

        new Command(serializeInstance(cloned, root), getClassBytes(root.class), supportsRoot + usedClosuresBytes + supportsUsedClosures)
    }
